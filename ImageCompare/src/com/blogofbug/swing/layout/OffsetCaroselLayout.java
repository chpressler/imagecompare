...
public class OffsetCaroselLayout extends CaroselLayout{
     ...
     */
    public OffsetCaroselLayout(Container forContainer, ContainerEffectManager componentEffects) {
        super(forContainer,componentEffects);
    }
    ...
    protected Point calculateCenter(Insets insets, int width, int height, int widest) {
        return new Point(0,((height-insets.bottom)/2));
    }
    ...
    protected float getFrontmostAlpha(float finalAngle) {
        float effectAlpha = (float) Math.sin(finalAngle);
